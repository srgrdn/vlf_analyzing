...
class PseudoEnvelopeCNN(nn.Module):
    def __init__(self):
        super().__init__()
...
def load_metadata(dataset_dir: Path) -> pd.DataFrame:
    ...
    if not metadata_path.exists():
        raise FileNotFoundError(f"{metadata_path} not found. Run prepare_threshold_dataset.py first.")
...
def build_loaders(metadata: pd.DataFrame, dataset_dir: Path, batch_size: int, num_workers: int = 0):
    ...
    loaders = {
        "train": DataLoader(PseudoEnvelopeDataset(train_df, dataset_dir), batch_size=batch_size, shuffle=True, num_workers=num_workers),
        "val": DataLoader(PseudoEnvelopeDataset(val_df, dataset_dir), batch_size=batch_size, shuffle=False, num_workers=num_workers),
        "test": DataLoader(PseudoEnvelopeDataset(test_df, dataset_dir), batch_size=batch_size, shuffle=False, num_workers=num_workers),
    }
...
def train(
    ...
    dataset_dir = (dataset_dir or method_dir / "dataset").resolve()
    ...
    model = PseudoEnvelopeCNN().to(device)
    ...
    report = {
        "model_kind": "threshold_pseudo_envelope_1d",
    ...
    model_path = models_dir / "threshold_pseudo_envelope_1d.pt"
    metrics_path = reports_dir / "threshold_pseudo_envelope_1d_metrics.json"
    predictions_path = reports_dir / "threshold_pseudo_envelope_1d_predictions.csv"

    torch.save({"model_kind": "threshold_pseudo_envelope_1d", "model_state_dict": model.state_dict(), "label_to_index": LABEL_TO_INDEX}, model_path)
    metrics_path.write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    predictions = frames["test"][["sample_id", "source_file", "segment_start", "segment_end", "label", "png_path", "npz_path", "peak_count", "threshold_db"]].copy()
