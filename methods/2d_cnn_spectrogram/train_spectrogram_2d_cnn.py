...
def build_loaders(metadata: pd.DataFrame, dataset_dir: Path, batch_size: int, num_workers: int = 0):
    ...
    loaders = {
        "train": DataLoader(SpectrogramDataset(train_df, dataset_dir), batch_size=batch_size, shuffle=True, num_workers=num_workers),
        "val": DataLoader(SpectrogramDataset(val_df, dataset_dir), batch_size=batch_size, shuffle=False, num_workers=num_workers),
        "test": DataLoader(SpectrogramDataset(test_df, dataset_dir), batch_size=batch_size, shuffle=False, num_workers=num_workers),
    }
...
def train(
    ...
    model_path = models_dir / "spectrogram_2d.pt"
    metrics_path = reports_dir / "spectrogram_2d_metrics.json"
    predictions_path = reports_dir / "spectrogram_2d_predictions.csv"

    torch.save({"model_kind": "spectrogram_2d", "model_state_dict": model.state_dict(), "label_to_index": LABEL_TO_INDEX}, model_path)
